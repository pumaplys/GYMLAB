...
|---|---|---|
| ~~**Texto de consentimiento en BORRADOR**~~ ✅ | **Resuelto el 2026-09-16.** La migración `0020` siembra la versión de lanzamiento `2026-09-16`, que **no** es borrador. El borrador anterior **no se aprobó**: se quedó como histórico, porque reescribir un texto aceptado destruye la prueba de que alguien lo aceptó. Ver [`docs/legal/`](legal/) | Hecho |
| **Identificador fiscal del prestador** 🔒 | Es el **único** bloqueo del gate `listo-para-tiendas`. No está en el repositorio a propósito: entra como `NEXT_PUBLIC_PRESTADOR_NIF` al construir la release. **No se inventa** | **Antes de publicar en tiendas** |
| **Revisión legal externa** 🔒 | Los documentos de `docs/legal/` los **adopta el responsable**; ninguno está revisado por un profesional, y ninguno afirma estarlo | **Antes de explotación comercial** |
| **pg-boss tumba el proceso al perder la conexión** 🔎 | Durante la verificación de #69 la API murió con `Connection terminated due to connection timeout` emitido por `pg-boss`; **PostgreSQL seguía en pie** y reiniciar la API lo resolvió. No parece relacionado con ese PR. Un cliente de colas que derriba el proceso entero al perder una conexión es algo a entender antes de tener clientes: en producción significaría caídas por un hipo de red. **Sin investigar todavía**, por decisión explícita | **En el cierre integral, antes de producción** |
