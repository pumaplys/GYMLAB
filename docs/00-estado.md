...
|---|---|---|
| **Texto de consentimiento en BORRADOR** 🔒 | El circuito está resuelto: hay plantilla de plataforma, documento inmutable por gimnasio y autoservicio del socio. Lo que falta **no es técnico**: (1) sustituir la plantilla `2026-09-01-borrador` por un texto revisado; (2) disponer de los datos del responsable que deban figurar —hoy el `controller` es el nombre comercial, porque `gyms` no tiene razón social ni NIF—; (3) publicar esa versión nueva. Publicarla **no toca las aceptaciones históricas**: los documentos son inmutables y las anteriores siguen apuntando a su texto | **Antes de tratar datos de salud reales** |
